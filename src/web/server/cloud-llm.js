// server/lib/cloud-llm.js
// Minimal "hosted LLM" adapter with 3 providers: OpenRouter, Gemini, Hugging Face.
...
export async function cloudComplete(prompt, { temperature = 0.2 } = {}) {
  ...
  if (provider === "hf" || provider === "huggingface") {
    ...
    const r = await fetch(`https://api-inference.huggingface.co/models/${encodeURIComponent(model)}`, {
      ...
      body: JSON.stringify({
        inputs: prompt,
        parameters: { max_new_tokens: 300, temperature }
      })
    });
    if (!r.ok) throw new Error(`hf HTTP ${r.status}`);
    const j = await r.json();
    // HF responses vary by model—handle common shapes:
    if (typeof j === "string") return j;
    if (Array.isArray(j)) {
      if (j[0]?.generated_text) return j[0].generated_text;
      if (j[0]?.summary_text) return j[0].summary_text;
    }
